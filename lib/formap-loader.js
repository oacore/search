...
const statsUrl = 'https://api.core.ac.uk/internal/formap'
  ...
  return new Promise((resolve, reject) => {
    if (stats == null) {
      fetch(url)
        .then((res) => {
          if (res.status >= 400)
            throw new Error('Could not fetch actual repositories')
          return res
        })
        .then((res) => res.json())
        .then((data) => {
...
const STATISTICS_CACHE_PATH = path.resolve('.formap.json')
...
export default async function getStatistics() {
  ...
  try {
    ...
    log('Formap cache not initialized')
  }
